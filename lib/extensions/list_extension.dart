...
extension FitListExtension<T extends Modelable> on List<T> {
  Model toModel({String id = "", String userId = ""}) => Model(
        ...
        data: {
          "data": [
            for (final data in this) data.toModel().toJson(),
          ],
        },
      );
  ...
  List<T> fromModel(Model model, T Function(Model) fromModelFactory) => (model
          .data["data"] as List<dynamic>)
      .map((e) => fromModelFactory(Model.fromJson(e as Map<String, dynamic>)))
      .toList();
}
